...
describe("Merkle AirdropNFT Token", () => {
  ...
  describe("Function initialize", () => {
    ...
    it("should revert if initialized with zero merkle root", async () => {
      ...
      await expect(
        airdropNft.initialize(sender.address, nft.address, zeroMerkleRoot)
    ...
    it("should only initialize the given value", async () => {
      ...
      await expect(
        airdropNft.initialize(sender.address, nft.address, badTree.getHexRoot())
      )
        .to.emit(airdropNft, "MerkleRootChanged")
  ...
  describe("Function claim", () => {
    ...
    it("should revert if user is not in merkle tree", async () => {
      await airdropNft.initialize(sender.address, nft.address, merkleRoot);
    ...
    it("should return false if not claimed", async () => {
      expect(await airdropNft.hasClaimed(users[0].address)).to.equal(false);
    });
    ...
    it("should revert if user already claimed", async () => {
      ...
      expect(await airdropNft.hasClaimed(users[0].address)).to.equal(true);
    ...
    it("should let user added in merkle tree to claim tokens", async () => {
      ...
      expect(await airdropNft.hasClaimed(users[2].address)).to.equal(true);
    });
  });
});
